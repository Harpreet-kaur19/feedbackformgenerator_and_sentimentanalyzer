...
answers in a single form's feedback. Results are cached in the
`sentiment_cache` SQLite table, keyed by response_id, so reloading the
...
def _load_cache(form_id: str) -> pd.DataFrame:
    with db_cursor() as cur:
        cur.execute(
            "SELECT response_id, sentiment, score, summary, keywords "
            "FROM sentiment_cache WHERE form_id = ?",
            (form_id,),
        )
        rows = cur.fetchall()
    if not rows:
        return pd.DataFrame(columns=CACHE_COLUMNS)
    return pd.DataFrame([dict(r) for r in rows], columns=CACHE_COLUMNS)


def _save_cache(form_id: str, df: pd.DataFrame) -> None:
    """Upsert every row currently in `df` into sentiment_cache. `df` is
    the full cache (existing rows + any newly-scored ones), so this is
    called after each analyze_form_feedback() run rather than on every
    single new row."""
    if df.empty:
        return
    with db_cursor() as cur:

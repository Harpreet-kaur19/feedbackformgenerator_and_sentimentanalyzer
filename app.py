...
from feedback.save_response import AlreadySubmittedError, already_submitted, load_responses, save_response
...
#
# "One submission per person" is enforced with a respondent cookie rather
# than a login: the first time someone opens/submits a form we hand their
# browser a random token, and remember (form_id, token) once they submit.
# It's not bulletproof (clearing cookies / a different browser resets it),
# but it needs no account system and matches how most public feedback
# forms behave.
# ============================================================================

def _respondent_token() -> str | None:
    """The respondent cookie already on this request, if any."""
    return request.cookies.get(Config.RESPONDENT_COOKIE_NAME)


def _with_respondent_cookie(response, token: str):
    """Attach/refresh the respondent cookie on an outgoing response."""
    response.set_cookie(
        Config.RESPONDENT_COOKIE_NAME,
        token,
        max_age=Config.RESPONDENT_COOKIE_MAX_AGE,
        httponly=True,
        samesite="Lax",
    )
    return response
...
def fill_form(form_id):
    """Render one specific generated form for users to fill out -- or,
    if this browser already submitted it, a "you already submitted"
    ...
    token = _respondent_token() or str(uuid.uuid4())
    submitted = already_submitted(form_id, token)

    response = make_response(
        render_template("fill_form.html", form=form, already_submitted=submitted)
    )
    return _with_respondent_cookie(response, token)


@app.route("/submit/<form_id>", methods=["POST"])
def submit(form_id):
    """Save a user's submitted answers, unless this browser already
    submitted this form."""
    form = load_form(form_id)
    if not form:
        flash("That form doesn't exist or was never generated.", "error")
    ...
    if already_submitted(form_id, token):
        flash("You have already submitted this form.", "error")
        response = make_response(redirect(url_for("fill_form", form_id=form_id)))
        return _with_respondent_cookie(response, token)
    ...
    try:
        save_response(form, answers, respondent_token=token)
    except AlreadySubmittedError:
        # Race: two submits from the same browser landed at once.
        flash("You have already submitted this form.", "error")
        response = make_response(redirect(url_for("fill_form", form_id=form_id)))
        return _with_respondent_cookie(response, token)
